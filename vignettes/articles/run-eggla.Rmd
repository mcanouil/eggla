...
1. Copy and edit the following code to a new file (_e.g._, `run_eggla.sh`) on the server that will run the analysis with the appropriate parameters.
  ...
  - `renv` (recommended):
    ...
    utils::download.file(
        url = "https://raw.githubusercontent.com/mcanouil/eggla/v0.8.1/inst/setup/renv.lock",
        destfile = "renv.lock"
      )
    renv::restore(lockfile = "renv.lock")
    renv::install("mcanouil/eggla@v0.8.1")
  ...
  - `pak` (faster):
    ...
    library(pak)
    utils::download.file(
      url = "https://raw.githubusercontent.com/mcanouil/eggla/v0.8.1/inst/setup/pkg.lock",
      destfile = "pkg.lock"
    )
    lockfile_install(lockfile = "pkg.lock", lib = temp_library)
    pkg_install("mcanouil/eggla@v0.8.1", lib = temp_library, upgrade = FALSE, dependencies = FALSE)
    EOF
...
2. Start R and setup working directory using `renv` (recommended) or `pak` (faster) to restore predefined version of packages
  ...
  - `pak` (faster):
    ...
    lockfile_install(lockfile = "pkg.lock", lib = temp_library)
    pkg_install("mcanouil/eggla@0.8.1", lib = temp_library, upgrade = FALSE, dependencies = FALSE)
    ```
