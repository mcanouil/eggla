...
1. Copy and edit the following code to a new file (_e.g._, `run_eggla.sh`) on the server that will run the analysis with the appropriate parameters.
  ...
  - `pak` (faster):
    ...
    pkg_install("mcanouil/eggla@v0.5.2", lib = temp_library, upgrade = FALSE, dependencies = FALSE)
    EOF
...
4. Run the GWASs on slopes and AUCs for male and female
    ...
    do_eggla_gwas(
      ...
      results_zip = list.files(pattern = "\\.zip", full.names = TRUE),
      id_column = "ID",
      formula = `slope_0--0.5` + `slope_1.5--5` + `slope_6--10` + `slope_12--17` + `auc_0--0.5` + `auc_1.5--5` + `auc_6--10` + `auc_12--17` ~ sex,
