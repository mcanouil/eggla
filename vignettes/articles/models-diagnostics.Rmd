...
random_effect <- c(
  ...
  "~ 1 | ID",
  "~ gsp(age, knots = c(1, 8, 12), degree = rep(3, 4), smooth = rep(2, 3)) | ID",
  "~ gsp(age, knots = c(1, 8, 12), degree = rep(3, 4), smooth = rep(2, 3))[, 1:3] | ID",
  "~ gsp(age, knots = c(1, 8, 12), degree = rep(2, 4), smooth = rep(2, 3)) | ID",
  "~ gsp(age, knots = c(1, 8, 12), degree = rep(1, 4), smooth = rep(2, 3)) | ID",
  "~ 1 | ID",
...
fixed_effect <- c(
  ...
  rep("log(bmi) ~ gsp(age, knots = c(1, 8, 12), degree = rep(3, 4), smooth = rep(2, 3))", 5),
...
all_models <- setDT(
  expand.grid(
    ...
    correlation_structure = c("NULL", "corCAR1(form = ~ 1 | ID)")[2], # Use AR(1) correlation structure (default is without)
...
```{r models-diagnostics-plot, dev.args = list(scaling = 0.50)}
wrap_plots(
  lapply(
...
```{r models-success-plot, fig.asp = 1}
