...
1. Copy and edit the following code to a new file (e.g., `run_eggla.sh`) on the server that will run the analysis with the appropriate parameters.
    ...
    Rscript \
      ...
      -e 'pak::lockfile_install(lockfile = system.file("setup", "pkg.lock", package = "eggla"))' \
      -e 'setwd(wd)' \
      -e 'library(eggla, lib.loc = temp_library)' \
      -e 'library(data.table, lib.loc = temp_library)' \
      -e 'res <- try(run_eggla(
...
2. Start R and setup working directory using `renv` to restore predefined version of packages
    ...
    pak::lockfile_install(lockfile = system.file("setup", "pkg.lock", package = "eggla"))
    ```
...
3. Run the analysis
    ...
    ```{r, eval = FALSE}
    setwd(wd)
    library(eggla, lib.loc = temp_library)
    library(data.table, lib.loc = temp_library)
    res <- try(
      run_eggla(
