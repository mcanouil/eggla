...
ggplot(data = pheno_dt, mapping = aes(x = age, y = .data[[trait]], colour = ID)) +
  ...
  stat_smooth(method = "loess", formula = y ~ x, linetype = 1, colour = "firebrick") +
...
* Exclusion:
    - Number of `r params[["trait"]]` observations: `r length(bmigrowth[["bmi"]]) - length(visits_clean[["bmi"]])`.
    - Number of individuals: `r length(unique(bmigrowth[["ID"]])) - length(unique(visits_clean[["ID"]]))`.
