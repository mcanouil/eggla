...
<!--     linetype = 1, colour = "firebrick", se = FALSE, na.rm = TRUE -->
<!--   ) + -->
<!--   scale_x_sqrt(limits = c(0, NA), expand = expansion(mult = c(0, 0.1))) -->
<!--   theme(legend.position = "none") + -->
<!--   labs( -->
<!--     x = paste0("AGE (", variables_unit[["age"]], ")"), -->
<!--     y = NULL -->
...
pheno_dt[j = "agedays" := floor(age * 365.25)] # convert to age in days and as integers ...
pheno_dt[j = "sex_daymont" := c("0" = "1", "1" = "0")[as.character(sex)]] # recode sex with Male = 0 and Female = 1...
pheno_dt[j = "WEIGHTKG" := as.numeric(weight)]
pheno_dt[j = "HEIGHTCM" := as.numeric(height)]
visits_long <- melt(
  data = pheno_dt, 
...
mres <- melt(
  data = visits_clean[, 
    list(
      cubic_slope = list(time_model(x = "age", y = trait_model, cov = params[["covariates"]], data = .SD, method = "cubic_slope")),
      linear_splines = list(time_model(x = "age", y = trait_model, cov = params[["covariates"]], data = .SD, method = "linear_splines")),
      cubic_splines = list(time_model(x = "age", y = trait_model, cov = params[["covariates"]], data = .SD, method = "cubic_splines"))
    ), 
    by = "sex"
  ], 
