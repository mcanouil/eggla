...
# tbl_cross(
#   data = pheno_tbl[j = list(ID, age, sex)],
#   row = "age",
#   col = "sex",
#   percent = "cell",
#   statistic = "{n} ({p} %)",
...
visits_qc_summary[
  ...
  by = seq_len(nrow(visits_qc_summary))
][
...
models_comp <- mres[
  j = (function(mods, names) {
    out <- eval(parse(text = paste0(
      "compare_performance(",
      paste(paste0("mods[[", seq_along(mods), "]]"), collapse = ", "),
...
ggplot(
  data = mres[
    j = (function(mr) {
      ...
      )[j = (trait) := revert_trans(predict(mr[[1]], .SD, level = 0)), .SDcols = "age"]
    })(model_results),
    by = c("sex", "model_name")
  ]
) +
