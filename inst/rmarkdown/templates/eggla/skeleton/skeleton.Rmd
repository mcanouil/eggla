...
```{r display-trajectories}
ggplot(data = pheno_dt, mapping = aes(x = age, y = .data[[trait]], colour = ID)) +
  geom_path(na.rm = TRUE, alpha = 0.25) +
  geom_point(size = 0.5, na.rm = TRUE, alpha = 0.25) +
  stat_smooth(method = "loess", formula = y ~ x, linetype = 1, colour = "firebrick", se = FALSE, na.rm = TRUE) +
...
* Exclusion:
    - Number of `r params[["trait"]]` observations: `r comma(abs(length(bmigrowth[["bmi"]]) - length(visits_clean[["bmi"]])))`.
    - Number of individuals: `r comma(abs(length(unique(bmigrowth[["ID"]])) - length(unique(visits_clean[["ID"]]))))`.
...
ggplot(
  data = mres[, 
    (function(mr) {
      data_pred <- data.table(age = seq(1, 15, 0.1))
      data_pred[, trait := revert_trans(predict(mr[[1]], .SD, level = 0)), .SDcols = "age"]
    })(model_results), 
    by = c("sex", "model_name")
  ]
) +
  aes(x = age, y = trait, colour = model_name, linetype = model_name) +
  geom_path() +
