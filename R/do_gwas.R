...
#' @param formula Formula to be used for the analysis,
#'   with on the left hand side, one or multiple traits to be analysed separately,
#'   and on the right hand side, the covariates to be used,
#'   *i.e.*, columns' names from `data`.
...
do_gwas <- function(
  ...
  traits <- unique(all.names(stats::as.formula(sprintf("~ %s", as.character(formula)[2])), functions = FALSE))
  covariates <- unique(all.names(stats::as.formula(sprintf("~ %s", as.character(formula)[3])), functions = FALSE))
  ...
  dt <- data.table::merge.data.table(
    ...
    .SDcols = c(id_column, all.names(formula, functions = FALSE))
  ]
